...
class ParticleAnimation extends StatefulWidget {
  ...
  State<StatefulWidget> createState() {
    return _ParticleAnimationState();
  }
}
...
class _ParticleAnimationState extends State<ParticleAnimation>
  ...
  void initState() {
    ...
    List.generate(numNodes, (i) {
      nodeList.add(Node(id: i, screenSize: widget.screenSize));
    });
    ...
    return Container(
      ...
      child: AnimatedBuilder(
        ...
        builder: (context, child) => CustomPaint(
          ...
          painter: _DemoPainter(widget.screenSize, nodeList),
        ),
      ),
    );
  }
...
class Node {
  ...
  void move(double seed) {
    switch (direction) {
      case Direction.LEFT:
        position -=  Offset(1.0 + seed, 0.0);
        if (position.dx <= 5.0) {
          var dirAvailableList = [
            Direction.RIGHT,
      ...
      case Direction.TOP_LEFT:
        position -= Offset(1.0 + seed, 1.0 + seed);
  ...
  bool canConnect(Node node) {
    var x = node.position.dx - position.dx;
    var y = node.position.dy - position.dy;
    var d = x * x + y * y;
